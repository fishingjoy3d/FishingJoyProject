		...
		if (mAtlas.spriteMaterial != null)
		{
			...
			if (sprite != null)
			{
				...
				if (tex != null)
				{
                    ...
                    if (GUILayout.Button("Cut All Sprites"))        //by zj 2016.9.19
                    {
                        string rootPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(mAtlas));//获取路径名称

                        if (!string.IsNullOrEmpty(rootPath))
                        {
                            if (!AssetDatabase.IsValidFolder(rootPath + "/" + mAtlas.name))//判断文件夹是否存在
                            {
                                AssetDatabase.CreateFolder(rootPath, mAtlas.name);//创建文件夹
                            }
                            ...
                            {
                                UIAtlasMaker.SpriteEntry se = UIAtlasMaker.ExtractSprite(mAtlas, mSpriteData.name);
